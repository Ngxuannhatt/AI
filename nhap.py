...
while True:
   ...
   # 1. Lấy tất cả hàng xóm

    all_neighbors = get_neighbors(current_state)

   

    # 2. LỌC: Chỉ giữ lại những hàng xóm CHƯA TỪNG đi qua

    valid_neighbors = [n for n in all_neighbors if n not in history]

   

    # Nếu không còn nước đi nào mới (tất cả đều đã đi qua hoặc vào ngõ cụt)

    if not valid_neighbors:

        print("khong con nuoc di moi nao khac.")
